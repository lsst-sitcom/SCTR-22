...
\textbf{ M1M3 Functional Re-verification and Integration with SAL v4.0},
...
\href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T246}{LVV-T246}
& {\small Andrew Serio [X] } & {\small  } &
\begin{minipage}[]{6cm}
\smallskip
{\small  }
\medskip
...
\begin{minipage}[]{6cm}
\smallskip
{\small {Probably requires some personnel on to ensure the safety of this test
but who is that specifically?}
...
\paragraph{ LVV-T246 - M13T-035: Actuator Replacement Time }\mbox{}\\

Version \textbf{1}.
Open  \href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T246}{\textit{ LVV-T246 } }
test case in Jira.

The objective of this test case is to verify that all types of actuators
for the M1M3 can be replaced in 6 hours or less. This test case will
incorporate a bump test in order to demonstrate the continuity with all
connections and the replacement actuator's ability to apply the correct
support forces. Furthermore, in order to provide the sufficient time for
setup, cleanup and calibration, the total time allocated to the physical
removal will be limited to 3 hours. This test case will only exercise
...
{I'm not sure if the scope of this test is saying that we should test
the replacement time of each type of actuator twice?}
...
\item
  ...
  the actuators}
\item
  {The M1M3 mirror system is in passive mode and the mirror is resting
  on the static supports. - This seems contradictory; are we just saying
  that this should have already been verified?}
\item
...
\tightlist
\item
  Position all required equipment outside of the cell. Since
  prepositioning can be effectively accomplished beforehand, this
  preparation time is not considered part of the total time.
\item
  Begin timing and start the replacement procedure
  \href{https://confluence.lsstcorp.org/display/~uhautmann}{Ulrike
  Hautmann} for link to procedure.
\item
  Note the time when the physical removal of the actuator has begun.
  This includes the disconnecting of all signal, electrical and
  pneumatic connection. This included all mechanical interfacing.~
\item
  Install one of the spare actuators, note the time when the physical
  installation of the actuator has been completed.~
\item
  Verify that each spare is a stand alone unit with all mechanical
  pneumatic and electronics.
\item
  Remove all installation equipment and start the calibration and
  rudimentary testing of the actuator. Using the force balance system
  and some commands (TBD) make sure everything is working as expected.
\item
  This testing need only be sufficient to demonstrate continuity with
  all connections and that the forces can be applied and measured
  according to the bump test. When the actuator has be certified as
  properly functioning stop timing.
\item
  Repeat the above test for two of each type of actuator. Do not remove
  any spares until all the actuators have been installed.
\end{enumerate}
...
\\ \cdashline{2-2}
 ...
 & Expected Result \\
 & \begin{minipage}[t]{15cm}{\footnotesize
Each type of actuator is expected to removed, replaced and fully
functional in 6 hrs.
...
CAID and SOML ahead of being shipped to Pachon.\\[2\baselineskip]{I've
traced an LVV-11363 here but it seems like it would get verified similar
to how LVV-11164. This makes me question the necessity of including this
test case as part of this cycle.}
...
The objective of this test is to verify the correct operation of the
electrical hardware associated to the interlock system. This test should
demonstrate that when the mirror cell door is opened, a signal is sent
to the GIS preventing any motion of the TMA and all power to the Fan
Coil Units is removed. This test case will only exercise this
...
\item
  Only requires the surrogate mirror to be installed
\item
  {Only requires the M1M3 IS to be connected to the GIS~}
\item
  Only requires the Fan Coil Units to be installed and outputting data
  to the EFD
\end{itemize}
...
\paragraph{ LVV-T234 - M13T-010: Position System Requirements (including IMS) }\mbox{}\\
